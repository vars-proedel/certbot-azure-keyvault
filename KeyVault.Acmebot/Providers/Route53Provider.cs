...
public async Task<IReadOnlyList<DnsZone>> ListZonesAsync()
{
    ...
    do
    {
        ...
        var response = await _amazonRoute53Client.ListHostedZonesAsync(request);

        zones.AddRange(response.HostedZones.Select(x => new DnsZone(this) { Id = x.Id, Name = x.Name.TrimEnd('.') }));
